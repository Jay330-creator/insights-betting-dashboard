...
- **Two-stage cost attribution.** Producer stamps run identifiers; cost sync attributes tokens later (decouples enrichment from LLM run completion).
...
- **Hard caps in AI prompts.** The 12pm preview prompt enforces max 8 tool calls and prefers `web_fetch` over browser. This single change cut preview cost by 79%.
...
A real incident log, because production systems break and how you handle them matters:
...
- **Score scraper returning dates.** A regex was matching MM-DD strings on a search results page (e.g., game date "5/4") and returning them as scores. Replaced with ESPN public API.
...
- **15× cost regression.** Sync crons were running as `agentTurn` (LLM) jobs to invoke deterministic scripts. Migrated them to macOS launchd, dropped daily cost from ~$1.71 to ~$0.30.
